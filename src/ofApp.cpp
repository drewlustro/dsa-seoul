...
void ofApp::setup(){
	...
	if (ENABLE_VIDEO) {
		video0.loadMovie("vail-gpro-4k-1.mp4");
		video1.loadMovie("vail-gpro-4k-2.mp4");
		//video0.loadMovie("vail-gpro-1080p-1.mov");
		video0.play();
		video1.play();
	}

	if (ENABLE_SHADER) {
		shader0.load( "liquifyShaderVert.c", "liquifyShaderFrag.c" );
		fbo0.allocate( ofGetWidth(), ofGetHeight() );

		shader1.load( "invertShaderVert.c", "invertShaderFrag.c" );
...
void ofApp::draw(){
	...
	if (ENABLE_SHADER) {
		...
		float time = ofGetElapsedTimef();
		shader0.setUniform1f( "time", time );	//Passing float parameter "time" to shader
		...
		fbo1.end();			//End drawing into buffer

		shader1.begin();		//Enable the shader
		shader1.setUniform1f( "time", time );	//Passing float parameter "time" to shader
		ofSetColor( 255, 255, 255 );

		if( winManager.getActiveWindowNo() == 1){
			fbo1.draw( 0, 0 );
		}
		...
		ofDrawBitmapString("Window " + ofToString(winManager.getActiveWindowNo()), -50.0f + ofGetWidth()/2, ofGetHeight()/2);
		ofDrawBitmapString(ofToString(ofGetFrameRate()) + " FPS", -50.0f + ofGetWidth()/2, ofGetHeight()/2 + 50.0f);
	}
